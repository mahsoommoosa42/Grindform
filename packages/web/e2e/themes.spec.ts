...
import { openApp, openThemePicker } from './helpers.ts';
...
for (const theme of THEMES) {
  test(`applies the ${theme} theme`, async ({ page }) => {
    await (await openThemePicker(page)).selectOption(theme);
    await expect(page.locator('html')).toHaveAttribute('data-theme', theme);
  });
}

test('persists the chosen theme across a reload', async ({ page }) => {
  await (await openThemePicker(page)).selectOption('girlypop');
  await expect(page.locator('html')).toHaveAttribute('data-theme', 'girlypop');
  await page.reload();
  await expect(page.locator('html')).toHaveAttribute('data-theme', 'girlypop');
  await expect(await openThemePicker(page)).toHaveValue('girlypop');
});
...
test('changing theme restyles the accent colour', async ({ page }) => {
  ...
  await picker.selectOption('grind');
  const grind = await accentOf();
  await picker.selectOption('girlypop');
